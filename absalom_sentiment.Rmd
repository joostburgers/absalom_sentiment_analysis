...
```{r}
    absalom_events <- read.csv("absalom_sentences_export_4_30_v2.csv")
```
...
#Tidy the begin_word vector by unnesting it into individual words. In the tidytext package, unnesting removes white spaces, punctuation, and special characters. The only special character that might remain is the curly quote [‘’]. These have a tendency to mess up the matching. Once all the words are clean, they are collapsed back into sentences.
...
#Join the tidied begin and end words back to the data_frame
...
#Convert the Absalom text to a tidy string. Here too the curly quotes have been removed as it distorted the matching. 
...
absalom_event_sentences <- absalom_events_tidy %>%
  mutate(event_sentence = str_match(
    ...
    #create a search string that reads "\\s*VAR1(.*?)\\s*VAR2". This essentially reads find the longest version of VAR1 and keep searching until you find the best possible match to VAR2. 
...
#FIXED: The regular expression matching is now much improved. In the csv files the order of events was not sorted by order in page. As a result, the string search could not search forward and find the end of the sentence. This needs to be fixed in the database itself. Second, some of the phrasing in the first 6-8 words were not verbatim from the text. These will eventually need to be fixed. Finally, one minor error is that some events are the actual length of the sentences they are looking for. This can't be fixed. In total, 5 events out of 644 do not properly match because of this issue. It is negligible. 

```

```{r tidy_event_sentences}
...
#Lemmatize the words and then inner_join the racial words, thereby dropping any Events that do not contain racial words. Calculate the count of racial words by EventID and pivot the table wider for each word.

absalom_race_words <- absalom_event_sentences_tidy %>% 
                      mutate(word_lemma = textstem::lemmatize_words(word)) %>% 
                      mutate(word_lemma = str_remove_all(word_lemma, "'s")) %>%
                      inner_join(race_words) %>% 
                      group_by(EventID, word_lemma)  %>% 
                      count(word_lemma) %>% 
                      pivot_wider(names_from = word_lemma, values_from = n, values_fill=0)
```
...
#Preliminary tests show no real difference between Present and mentioned other than an ovious occurrence difference. 
absalom_race_present <- absalom_race_events %>% 
                          filter(PresentMentioned=="Present") %>% 
                          group_by(CharacterName) %>% 
                           summarise(across(black:miscegenation, ~sum(.)))

absalom_race_mentioned <- absalom_race_events %>% 
...
fig_race_bar <-
  fig_race_bar %>% add_trace(y = ~ blood, name = 'Blood')
fig_race_bar <-
  fig_race_bar %>% add_trace(y = ~ negro, name = 'Negro')
fig_race_bar <-
  fig_race_bar %>% add_trace(y = ~ nigger, name = 'Nigger')
fig_race_bar <-
  fig_race_bar %>% add_trace(y = ~ white, name = 'White')
...
```{r race_character_prop}
character_occurrence <-  absalom_events_short %>% 
                          count(CharacterName, name="char_count")

absalom_race_char_prop <- absalom_race_character %>% 
                          #left_join(character_occurrence,by="CharacterName") %>% 
                          # arrange(desc(char_count)) %>% 
                            top_n(5) %>% 
                          mutate(across(black:miscegenation, .fns = ~(. / char_count)))
```
...
absalom_race_n_prop <- absalom_race_char_prop %>%
  top_n(5) %>%
  select(1:3, 5:6, 10)
...
fig_race_bar_prop <-
  fig_race_bar_prop %>% add_trace(y = ~ blood, name = 'Blood')
fig_race_bar_prop <-
  fig_race_bar_prop %>% add_trace(y = ~ negro, name = 'Negro')
fig_race_bar_prop <-
  fig_race_bar_prop %>% add_trace(y = ~ nigger, name = 'Nigger')
fig_race_bar_prop <-
  fig_race_bar_prop %>% add_trace(y = ~ white, name = 'White')

fig_race_bar_prop <- fig_race_bar_prop %>%
  layout(
    title = "Frequency of Racial Words in Events with Specific Characters",
    yaxis = list(title = 'Percentage of Events',
                 tickformat= ',.0%'),
    xaxis = list(title = 'Character'),
    barmode = 'group',
    colorway = faulkner_colorway
