...
```{r load_race_words, cache=TRUE}
#Read in the race words
race_words <- read_csv("race_words.csv")
```

```{r _load_event_sentences, cache=TRUE}
...
tags$h6(HTML("<i>Figure 4:</i> The above bubble chart shows the frequency of racial word use by chapter. Immediately noticeable is that the overwhelming majority of the use of hte word 'nigger' appear in chapter 7."), style =fig_caption)
...
tags$h6(HTML("<i>Figure 5:</i> The line chart shows the positive and negative of sentences. Whenever a sentence had a racial word in it, it was coded as orange."), style =fig_caption)
tags$br()
...
n_word_character_plot <- n_word_character_plot %>%
  layout(
    title = "Ten Most Frequent Characters in Events <br> when the N-word is Used",
...
The chart reveals, quite dramatically, that Thomas Sutpen is in some way part of the event `r round((Sutpen_n_word_percent$nigger[1])*100,0)`% of the times that the n-word occurs. While it is not clear that he is using the word, it is clear that he is the character with whom the racial epithet is most associated. Indeed, in one event it occurs 16 times. This is when Sutpen is barred from entering the front door of the plantation by the enslaved butler (187). It is this primal incident that shapes so much of Sutpen's consciousness going forward. Linguistically, it becomes the gravitational center that draws in the worst racial language American literature has to offer. At the moment that he becomes aware of his class difference, he resorts to racial antagonism.
...
Don't have a good conlcusion yet. 

The linguistic analysis of Faulkner's use of racial language is quite revealing. It demonstrates that in his work he does not use the n-word casually. Instead, it serves a very specific purpose as a dramatic and thematic marker. It frames the speaker, the spoken to, and narrator who is re-speaking. By concentrating the n-word in one section and with respect to one specific incident, in one place, with one particular person the language appears contained, but it is anything but. It spreads throughout the text and infects everyone who re-tells the story. Even if the speakers don't repeat the word, they are implicated in the violence that brought it to life as one of the most problematic words in the English language.  
