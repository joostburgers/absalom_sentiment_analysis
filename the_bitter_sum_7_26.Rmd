...
The following is a brief linguistic analysis of the use of racially charged language created for the Norton Critical Edition of William Faulkner's *Absalom, Absalom!*. While it serves the supplement the introductory essay by Susan Scott Parrish, it also serves as an example of the way in which the *Digital Yoknapatawpha* data sets can be combined with other computational techniques to illuminate different aspects of Faulkner's work.
...
#TODO CONNECT TO ZOTERO BIBLIOGRAPHY AND INSERT HERE.

Given this extensive critical history, it almost goes without saying that a computational analysis of word choice, especially with regard to racially charged language, cannot do justice to the complexities and nuances of either the text or Faulkner's broader critical intervention. Nevertheless, using techniques common in Corpus Linguistics (CL) it is possible to give a bird's-eye view of how the use of certain words is patterned. This pattern, can then in turn, inform subsequent close-readings. 

The analysis demonstrates that Faulkner's use of racial language is not incidental but carefully patterned. This is particularly pronounced with his use of the word "nigger" - perhaps the most charged word in American English. Faulkner makes extensive use of this word throughout this text. As the data reveal, the n-word occurs most frequently in relation to Thomas Sutpen, and in particular his origin story in the 1830s in Tidewater, Virginia. When he is unceremoniously rebuffed at the front entrance of a plantation by an enslaved butler, he becomes simultaneously race and class conscious . The scene, and Sutpen's origin writ large, will replay themselves through the voices of the text's narrators. Although distinct, it would be a mistake to see these voices as singular. The linguistic analysis reveals overlapping patterns between narrators, indicating that while there may be differences in tone, register, and inflection the story material is communal. The narrator's do not own their voice any more than they own another person's story. They are the bard's of its transmission. 
...
- **Part 3: The Use of Racial Words by Characters, Time Period, and Locations**  
...
## Part 2: Character Word Analysis
...
shorter_nword_chapter7_consolidated <- nword_chapter7_consolidated %>% 
                                        mutate(theme = ifelse(str_detect(event, "Wash")==TRUE, "1861 - 1869 Wash", event)) %>% 
                                        filter(n_word>0)
...
plot_chapter7_consolidated <- plot_ly(shorter_nword_chapter7_consolidated,
                         ...
                         x = ~page,
                        color = ~theme,
                       colors=c("#132C53","#F27A18","#ae0700","#b4739a","#38726c","#e6d812","#448b2d"),
                       opacity = .9,
